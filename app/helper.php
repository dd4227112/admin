...
function createRoute()
{
    ...
    $controller = isset($url_param[2]) && !empty($url_param[2]) ? ucfirst($url_param[2]) : 'Analyse';
    $method = isset($url_param[3]) && !empty($url_param[3]) ? $url_param[3] : 'index';
    $view = $method == 'view' ? 'show' : $method;
    // Check if controller is in the allowed list
    if (in_array($controller, array('public', 'storage'))) {
        return null; // Return null for invalid controllers
    }
