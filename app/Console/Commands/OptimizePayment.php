...
//            }
//        }
        $students = DB::table('shulesoft.store_students_id')->where('status', 0)->whereNot('schema_name', 'stjohnbosco')->limit(9)->get();
        //check if all payments has been uploaded
        foreach ($students as $student) {
            if (!empty($student)) {
                $student_id = $student->student_id;
              
                if (DB::SELECT("SELECT * FROM shulesoft.redistribute_student_payments($student_id, '{$student->schema_name}')")) {
                    $update = ['status' => 1];
