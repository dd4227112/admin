...
public function syncMissingPayments($data, $schema, $student = null, $amount = null, $date = '') {
    ...
    if(isset($status->status) && $status->status == 0){ 
        ...
        $sms = 'Hello Mr. Endobile this Invoice '. $reference . ' of '.$student.' from '. $schema .' with paid amount of '. $amount .' failed to be paid. With Error message: '.$message.' happened on '.$date.' Take a look';
