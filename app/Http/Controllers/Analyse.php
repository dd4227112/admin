...
class Analyse extends Controller {
    ...
    public function search() {
       ...
       $schools = DB::select(" select  * from (select  name as sname, username as schema_name, 'default.png' as photo, CASE WHEN EXISTS (SELECT 1 FROM admin.client_schools a WHERE a.client_id = c.id and c.status is not null) THEN 1 ELSE (select id from admin.schools where lower(name) ilike '%bama%' limit 1 ) END AS is_schema from admin.clients c where  LOWER(username) LIKE '%" . $q . "%' union select name as sname, name as schema_name,'default.png' as photo, id as is_schema from admin.schools where lower(name) like '%" . $q . "%') b order by is_schema asc limit 10");
