(function () {
    ...
    var db = {
        ...
        loadData: function (filter) {
            return $.grep(this.clients, function (client) {
                return (!filter.region || client.region.indexOf(filter.region) > -1)
                        && (!filter.district || client.district === filter.district)
                        && (!filter.ward || client.ward.indexOf(filter.ward) > -1)
                        && (!filter.name || client.name === filter.name)
                        && (!filter.type || client.type === filter.type)
                 && (!filter.ownership || client.ownership === filter.ownership)
                        //&& (filter.ownership === undefined || client.ownership === filter.ownership);
            });
        },
   ...
   $.getJSON('http://localhost/shulesoft/market/getSchools',function(data){
         db.clients = data;
    });
    db.clien = [
        {
