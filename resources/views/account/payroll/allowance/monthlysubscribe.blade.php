                        ...
                        if (!empty($users)) {
                            ...
                            foreach ($users as $user) {
                                ...
                                <tr id="std<?= $user->id; ?>">
                                    ...
                                    <td data-title="<?= __('student_section') ?>">
                                        <?php
                                        $allowance_ = \App\Model\UserAllowance::where('user_id', $user->id)->where('table', $user->table)->where('deadline', '>', date('Y-m-d'))->first();
                                    ...
                                    <td data-title="<?= __('student_section') ?>">
                                        <input  type="text" class="form-control calendar" id="deadline<?= $user->id ?>" name="amount" value="<?= $deadline ?>" >
                                    </td>
...
<script type="text/javascript">
    function submit_deduction(a, b) {
        ...
        $.ajax({
            ...
            data: {user_id: a, table: b, amount: amount, deadline: deadline, allowance_id: '<?= $set ?>', type: 0},
            dataType: "html ",
    ...
    function remove_user(a, b) {
        $.ajax({
            ...
            data: {user_id: a, table: b, set: '<?= $set ?>', type: 'allowance'},
            dataType: "html ",
            beforeSend: function (xhr) {
                $('#stat' + a + b).html('<a href="#/refresh"><i class="fa fa-spinner"></i> </a>');
            },
