                                        ...
                                        if ((int) $request->bank_approved == 1) {
                                            if (!empty($request)) {
                                                ...
                                                if ((int) $client->is_new_version == 1) {
                                                  ...
                                                  if (empty($integrated) ){
                                                    $bank_id = \DB::select("select id from shulesoft.bank_accounts where uuid =(select uuid from ".$request->client->username.".bank_accounts where id =".$request->bank_account_id.") limit 1 ");
                                                    $integrated = \DB::table('shulesoft.bank_accounts_integrations')->where('id', $bank_id)->where('schema_name', $request->client->username)->first();
                                                  }
