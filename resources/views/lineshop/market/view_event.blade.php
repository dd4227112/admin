...
<?php $root = url('/') . '/public/' ?>
    ...
    <!-- Page-header end -->
    <?php
    $medias = \App\Models\LineshopEventAttendee::where('event_id', $event->id)->get();
    ?>
    <!-- Page-body start -->
    <div class="page-body">
      <div class="row">
                                ...
                                <tbody>
                                  <?php if (sizeof($medias) > 0) {
                                    ...
                                    foreach ($medias as $media) {
                                      $pharmacy = (int)($media->pharmacy_id) > 0 ? $media->pharmacy->name : $media->pharmacy_id;
                                      ?>
                                      <tr>
                                        ...
                                        <td> <a class="btn btn-danger btn-sm" href="{{ url('lineshop/deleteUser/'.$media->id) }}">Delete</a></td>
                                      </tr>
                                    <?php $i++;} } ?>
                                  </tbody>
                                </table>
        ...
        $(".mark").blur(function (event) {
          ...
          if (inputs == null) {
            ...
            $.ajax({
              type: 'POST',
              url: "<?= url('lineshop/socialMediaUpdate') ?>",
              data: {
                "type_id": type_id,
