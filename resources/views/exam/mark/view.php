    ...
    <div class="well">
        <div class="row">
            <div class="col-sm-6">
                ...
                <?php
                 echo btn_add_pdf('mark/print_preview/'.$student->student_id."/".$set, $data->lang->line('pdf_preview')) 
                ?>
            ...
            <div class="col-sm-6">
                <ol class="breadcrumb">
                    ...
                    <li><a href="<?=base_url("mark/index/$set")?>"><?=$data->lang->line('menu_mark')?></a></li>
                    <li class="active"><?=$data->lang->line('view')?></li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <div id="printablediv">
        <section class="panel">
            <div class="profile-view-head">
                <a href="#">
                    <?=img(base_url('storage/uploads/images/'.$student->photo))?>
                </a>

                <h1><?=$student->name?></h1>
                <p><?=$data->lang->line("student_classes")." ".$classes->classes?></p>

            </div>
            <div class="panel-body profile-view-dis">
                <h1><?=$data->lang->line("personal_information")?></h1>
                <div class="row">
                    <div class="profile-view-tab">
                        <p><span><?=$data->lang->line("mark_roll")?> </span>: <?=$student->roll?></p>
                    </div>
                    <div class="profile-view-tab">
                        <p><span><?=$data->lang->line("menu_section")?> </span>: <?php if(count($section)) { echo $section->section;} else { echo $student->section->section;}?></p>
                    </div>
                    <div class="profile-view-tab">
                        <p><span><?=$data->lang->line("mark_dob")?> </span>: <?=date("d M Y", strtotime($student->dob))?></p>
                    </div>
                    <div class="profile-view-tab">
                        <p><span><?=$data->lang->line("mark_sex")?> </span>: <?=$student->sex?></p>
                    </div>
                    <div class="profile-view-tab">
                        <p><span><?=$data->lang->line("mark_religion")?> </span>: <?=$student->religion?></p>
                    </div>
                    <div class="profile-view-tab">
                        <p><span><?=$data->lang->line("mark_email")?> </span>: <?=$student->email?></p>
                    </div>
                    <div class="profile-view-tab">
                        <p><span><?=$data->lang->line("mark_phone")?> </span>: <?=$student->phone?></p>
                    </div>
                    <div class="profile-view-tab">
                        <p><span><?=$data->lang->line("mark_address")?> </span>: <?=$student->address?></p>
                    </div>
                    <?php if(can_access('view_student')) { ?>
                    <div class="profile-view-tab">
                        <p><span><?=$data->lang->line("mark_username")?> </span>: <?=$student->username?></p>
                    </div>
                    <?php } ?>
                </div>

                <h1><?=$data->lang->line("mark_information")?></h1>
                                    ...
                                    foreach ($exams as $exam) {
                                            ...
                                            if($exam->examID <= $max_semester) {
                                                ...
                                                if($check>=0) {
                                                    ...
                                                    echo "<thead>"; 
                                                        echo "<tr>";
                                                            echo "<th>";
                                                                echo $data->lang->line("mark_subject");
                                                            echo "</th>";
                                                            echo "<th>";
                                                            ...
                                                            if(count($grades) && $f == 1) {
                                                                ...
                                                                echo "</th>";
                                                                echo "<th>";
                                                                    echo $data->lang->line("mark_grade");
                                                                echo "</th>";
                                                            }
                                                        echo "</tr>";
                                                    echo "</thead>";
                                        ...
                                        foreach ($marks as $mark) {
                                            if($exam->examID == $mark->examID) {
                                                echo "<tr>";
                                                    echo "<td data-title='".$data->lang->line('mark_subject')."'>";
                                                        echo $mark->subject;
                                                    echo "</td>";
    ...
    <script language="javascript" type="text/javascript">
        ...
        function check_email(email) {
            ...
            if (email.search(emailRegEx) == -1) {
                $("#to_error").html('');
                $("#to_error").html("<?=$data->lang->line('mail_valid')?>").css("text-align", "left").css("color", 'red');
            } else {
                status = true;
            }
            return status;
        ...
        $("#send_pdf").click(function(){
            ...
            if(to == "" || to == null) {
                error++;
                $("#to_error").html("");
                $("#to_error").html("<?=$data->lang->line('mail_to')?>").css("text-align", "left").css("color", 'red');
            } else {
            ...
            if(subject == "" || subject == null) {
                error++;
                $("#subject_error").html("");
                $("#subject_error").html("<?=$data->lang->line('mail_subject')?>").css("text-align", "left").css("color", 'red');
            } else {
                $("#subject_error").html("");
            }
